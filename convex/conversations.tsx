...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const conversationsWithDetails = await Promise.all(conversations.map(async (conversation, index) => {
            ...
            const lastSeenMessageId = conversationMemberships[index].lastSeenMessageId;
            const lastSeenMessage = lastSeenMessageId ? await ctx.db.get(lastSeenMessageId) : null;
            ...
            if (conversation.isGroup) {
                ...
                const otherMembership = allMemberships.find(membership => membership.memberId !== currentUser._id);
                if (otherMembership) {
